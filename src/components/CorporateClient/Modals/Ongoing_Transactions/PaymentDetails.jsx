...
const PaymentDetails = ({show}) => {
  const { modal, closeModal, updateModal } = useModal();
  return (
    <>
      <div className="relative px-6 py-3 flex-auto">
        <h2 className="text-bg__color font-Roboto text-xl font-semibold mb-4">
          Payment Details
        </h2>
        ...
        {show ? (
          <>
            ...
            <div className="md:flex lg:flex-row mt-4 mb:4 flex-col">
              <div className="lg:w-1/2 sm:w-full mr-1">
                <button className="bg-bg__color w-full text-center block text-white px-4 py-1 rounded">
                  Submit
                </button>
              </div>
            </div>
        ...
        {modal.state === "ShowPaymentProof" && (
          <Image_modal title={"Payment Details"} closemodel={closeModal} />
        )}
      </div>
    </>
  );
